        ...
        return SequenceFactory.CreateFromIterator(
            _ => {
                ...
                var originalType = Array.Find(new[]{                    
                    ...
                    ValueType.XsDecimal,
                    ValueType.XsDouble,
                    ValueType.XsFloat}, type => firstValue.GetValueType().IsSubtypeOf(type));

                var itemAsDecimal = firstValue.CastToType(ValueType.XsDecimal).GetAs<AtomicValue>();
                var scaling = Math.Pow(10, scalingPrecision);
                var roundedNumber = DetermineRoundedNumber(Convert.ToDecimal(itemAsDecimal.GetValue()), halfToEven, (decimal)scaling);
                return originalType switch
                {
                    ValueType.XsDecimal => IteratorResult<AbstractValue>.Ready(AtomicValue.Create(roundedNumber, ValueType.XsDecimal)),
                    ValueType.XsDouble => IteratorResult<AbstractValue>.Ready(AtomicValue.Create(roundedNumber, ValueType.XsDouble)),
                    ValueType.XsFloat => IteratorResult<AbstractValue>.Ready(AtomicValue.Create(roundedNumber, ValueType.XsFloat)),
                    ValueType.XsInteger => IteratorResult<AbstractValue>.Ready(AtomicValue.Create(roundedNumber, ValueType.XsInteger)),
                    _ => throw new ArgumentOutOfRangeException()
                };
            }
        );
