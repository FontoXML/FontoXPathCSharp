...
public class BucketUtils
{
    ...
    public static string[] CreateBuckets(NodeType nodeType, string? localName)
    {
        ...
        if (nodeType == NodeType.Attribute || nodeType == NodeType.Element) {
            buckets.Add("type-1-or-type-2");
        }

        buckets.Add($"type-{GetBucketTypeId(nodeType)}");

        if (localName != null) {
            buckets.Add($"name-${localName}");
        }
    ...
    public static string[] GetBucketsForNode<TNode>(TNode node, IDomFacade<TNode> domFacade)
    {
        ...
        if (nodeType == NodeType.Attribute || nodeType == NodeType.Element)
        {
            localName = domFacade.GetLocalName(node);
        }
    ...
    private static IReadOnlyDictionary<string, string[]> SubBucketsByBucket = new Dictionary<string, string[]>
    {
    ...
    public static string? IntersectBuckets(string? bucket1, string? bucket2)
    {
        ...
        if (SubBucketsByBucket.ContainsKey(type1) && SubBucketsByBucket[type1].Contains(type2)) {
            // bucket 2 is more specific
            return bucket2;
        }
        if (SubBucketsByBucket.ContainsKey(type2) && SubBucketsByBucket[type2].Contains(type1)) {
